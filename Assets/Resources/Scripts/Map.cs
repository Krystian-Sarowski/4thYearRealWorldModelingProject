...
public class Map : MonoBehaviour
{
    ...
    string LoadTileSprite(MapIndex t_mapIndex)
    {
        ...
        return "Sprites/TileSprites/Tile";
    }
    ...
    void UpdateCharacterInMap(GameObject t_charEntity)
    {
        ...
        if (collider != null)
        {
                    ...
                    corner.m_y >= 0 && corner.m_y < m_hight)
                {
                    if (!m_grid[corner.m_x][corner.m_y].GetEntityList().Contains(t_charEntity))
                    {
                        ...
                        if (!m_grid[corner.m_x][corner.m_y].GetIsTraversable())
                        {
                            ...
                            if (t_charEntity.tag == "Player")
                            {
                                if (CheckIsDoorOpen(corner))
                                {
            ...
            else if (outOfBounds)
            {
                ...
                if (t_charEntity.tag == "Player")
                {
                    PlayerController playControl = t_charEntity.GetComponent<PlayerController>();
                    playControl.StopMovement();
                }

                // Enemy out of bounds 
                if(t_charEntity.tag == "Character")
                {
                    EnemyController enemyControl = t_charEntity.GetComponent<EnemyController>();
                    enemyControl.StopMovement();
                }
    ...
    /// <summary>
    /// Converts the map index position into world position
    /// which is at the centre of the tile at the map index position.
    /// </summary>
    /// <param name="mapIndex">The index position within the map</param>
    /// <returns>The world position of the map index position at the centre of the tile</returns>
    public Vector2 MapIndexToWorldPosition(MapIndex mapIndex)
    {
        Vector2 worldPos = new Vector2(mapIndex.m_x * m_tileSize + m_tileSize / 2, mapIndex.m_y * m_tileSize + m_tileSize / 2);
    ...
    /// <summary>
    /// Check if the door is open by checking if
    /// the tile contains a door entity and if that door is open.
    /// </summary>
    /// <param name="t_index">The index position of the tile</param>
    /// <returns>A bool for if a door is found and in a tile and it is open</returns>
    public bool CheckIsDoorOpen(MapIndex t_index)
    {
        List<GameObject> entityList = m_grid[t_index.m_x][t_index.m_y].GetEntityList();

        foreach(GameObject entity in entityList)
        {
            if(entity.tag == "Door")
            {
                if(entity.GetComponent<Door>().GetIsOpen())
                {
                    return true;
                }
            }
