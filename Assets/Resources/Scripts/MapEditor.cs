...
public class MapEditor : MonoBehaviour
{
    ...
    public void InstantiateMapObject(MapIndex t_startMapIndex, MapObject t_mapObject)
    {
        ...
        gameObject.transform.position = m_map.MapIndexToWorldPosition(t_startMapIndex) + positionOffset;
    ...
    {
        GameObject loadedPrefab = Resources.Load(t_mapObject.m_prefabPath, typeof(GameObject)) as GameObject;

        Vector2 positionOffset = new Vector2(m_map.m_tileSize * (t_mapObject.m_width - 1) / 2,
           m_map.m_tileSize * (t_mapObject.m_height - 1) / 2);

        GameObject gameObject = Instantiate(loadedPrefab, m_map.MapIndexToWorldPosition(t_startMapIndex) + positionOffset, Quaternion.identity);
    ...
    {
        string path = "Prefabs/" + t_fileName;
